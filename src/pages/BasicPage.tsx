...
---
욕조에 물을 채우고 물결을 만들어서 직접 실험해보셔도 알 수 있겠지만, 파동이 틈을 통과하면 그 틈이 기준이 되어 원 모양으로 다시 퍼져나가게 됩니다. 벽에 그려진 선은 벽에 도달한 파동의 세기를 표시한 것입니다.

그럼 이제 조금 더 어려운 상황을 보도록 할까요?
`.paragraph(DoubleWaveGraphic)`
아까 공 던지기 실험에서 보았던 이중 슬릿입니다. 이 이중 슬릿에 파동을 통과시키면 파동은 어떻게 진행하고, 벽에는 어떤 무늬가 나타날까요? 쉽지는 않겠지만, 한 번 생각해보세요!
---
파동이 두 개의 틈을 통과하면, 왼쪽 슬릿에서 생기는 파동과 오른쪽 슬릿에서 생기는 파동이 동시에 진행합니다. 두 파동이 서로 만나면 상쇄되거나 보강되기 때문에 조금 더 복잡한 모양의 파동이 만들어집니다. 이렇게 여러 개의 파동이 만나 위상이 달라지는 현상을 "파동의 간섭"이라고 합니다.

그리고 공을 이중 슬릿에 던졌을 때는 두 줄의 띠가 나타났는데, 파동의 경우는 조금 다른 무늬가 나타납니다. 이 무늬를 "간섭 무늬"라고 합니다.
