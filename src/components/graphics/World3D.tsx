...
export default function World3D({ children }: { children: ReactNode }) {
  ...
  return (
    <div ref={ref} className="world3d">
      <GraphicsBoundary>
        <Canvas shadows={!smallScreen} dpr={smallScreen ? 1 : [1, 1.75]} frameloop={running ? "always" : "demand"}
          gl={{ antialias: true, powerPreference: "default" }} fallback={null}>
          <SpringContext value={{ pause: !running && !reducedMotion, immediate: reducedMotion }}>
            <Camera />
